import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import Setting from "@/config"

export default function Home() {
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* AI Performance Evaluation Card */}
          <Card className={`${Setting.cardBackgroundGradient} border-border relative overflow-hidden min-h-[380px]`}>
            <div className="absolute bottom-5 right-4 flex items-center justify-center z-0 opacity-100">
              <svg xmlns="http://www.w3.org/2000/svg" width="284" height="284" viewBox="0 0 24 24">
              <title>star-sparkle</title>
              <g fill="none">
                <path d="M20.3174 8.19531C20.0357 8.8823 19.5416 9.61008 18.9248 10.332C17.6628 11.8091 15.7312 13.4472 13.4355 14.9248C11.14 16.4022 8.8494 17.4826 6.98242 18.0195C6.31873 18.2104 5.67868 18.3394 5.09375 18.3857L5.45996 16.3232C5.74837 16.2727 6.07264 16.1993 6.42969 16.0967C8.06215 15.6272 10.1747 14.6455 12.3535 13.2432C14.5326 11.8406 16.3019 10.3247 17.4053 9.0332C17.7661 8.61089 18.0377 8.22999 18.2305 7.90234L20.3174 8.19531Z" fill="url(#1752671328339-4791743_star-sparkle_existing_0_t5afbmhh9)" data-glass="origin" mask="url(#1752671328339-4791743_star-sparkle_mask_zol1sdobn)"></path>
                <path d="M20.3174 8.19531C20.0357 8.8823 19.5416 9.61008 18.9248 10.332C17.6628 11.8091 15.7312 13.4472 13.4355 14.9248C11.14 16.4022 8.8494 17.4826 6.98242 18.0195C6.31873 18.2104 5.67868 18.3394 5.09375 18.3857L5.45996 16.3232C5.74837 16.2727 6.07264 16.1993 6.42969 16.0967C8.06215 15.6272 10.1747 14.6455 12.3535 13.2432C14.5326 11.8406 16.3019 10.3247 17.4053 9.0332C17.7661 8.61089 18.0377 8.22999 18.2305 7.90234L20.3174 8.19531Z" fill="url(#1752671328339-4791743_star-sparkle_existing_0_t5afbmhh9)" data-glass="clone" filter="url(#1752671328339-4791743_star-sparkle_filter_0fcp7fp8u)" clip-path="url(#1752671328339-4791743_star-sparkle_clipPath_mpnb7o899)"></path>
                <path d="M10.6642 2.61922C11.2208 1.52816 12.7794 1.52811 13.336 2.61922L15.5069 6.87508C15.7292 7.31078 16.1505 7.6107 16.6349 7.67879L21.4083 8.34969C22.654 8.52479 23.1438 10.0637 22.2286 10.9268L18.839 14.1231C18.4713 14.4699 18.3034 14.979 18.3917 15.4766L19.2003 20.0372C19.4164 21.2558 18.1473 22.1966 17.0441 21.6358L12.6798 19.418C12.2526 19.2009 11.7476 19.2009 11.3204 19.418L6.95616 21.6358C5.85285 22.1967 4.58376 21.2559 4.79991 20.0372L5.60851 15.4766C5.69678 14.9789 5.52801 14.4699 5.16027 14.1231L1.77159 10.9268C0.856418 10.0637 1.34619 8.52479 2.59191 8.34969L7.36534 7.67879C7.84961 7.61064 8.27006 7.31071 8.4923 6.87508L10.6642 2.61922Z" fill="url(#1752671328339-4791743_star-sparkle_existing_1_msjgnlsec)" data-glass="blur"></path>
                <path d="M10.6641 2.61911C11.2207 1.52809 12.7793 1.52809 13.336 2.61911L15.5079 6.87498C15.7301 7.31065 16.1505 7.61058 16.6348 7.67869L21.4083 8.34959C22.654 8.5247 23.1438 10.0637 22.2286 10.9267L18.839 14.123C18.4713 14.4698 18.3034 14.9789 18.3917 15.4766L19.2003 20.0371C19.4097 21.2177 18.2249 22.1382 17.1475 21.6846L17.044 21.6367L12.6797 19.418C12.2525 19.2008 11.7475 19.2008 11.3203 19.418L6.95602 21.6367L6.85251 21.6846C5.80984 22.1236 4.6659 21.2757 4.78315 20.1504L4.79975 20.0371L5.60835 15.4766C5.68559 15.0411 5.56634 14.5972 5.28901 14.2607L5.16108 14.123L1.7714 10.9267C0.884703 10.0905 1.31671 8.62007 2.47746 8.3701L2.59172 8.34959L7.36521 7.67869C7.84952 7.61057 8.26991 7.31064 8.49217 6.87498L10.6641 2.61911ZM12.668 2.95993C12.3897 2.41442 11.6104 2.41442 11.332 2.95993L9.16015 7.2158C8.82676 7.86931 8.19619 8.31969 7.4697 8.42185L2.69621 9.09275C2.07335 9.18031 1.82846 9.9493 2.28605 10.3808L5.67574 13.5771C6.22733 14.0974 6.47906 14.8609 6.34664 15.6074L5.53804 20.168C5.42998 20.7772 6.06456 21.248 6.61618 20.9678L10.9805 18.749C11.6212 18.4234 12.3789 18.4234 13.0196 18.749L17.3839 20.9678C17.9355 21.248 18.5701 20.7772 18.462 20.168L17.6534 15.6074C17.521 14.8609 17.7727 14.0974 18.3243 13.5771L21.714 10.3808C22.1716 9.9493 21.9267 9.18031 21.3038 9.09275L16.5303 8.42185C15.8038 8.31969 15.1733 7.86931 14.8399 7.2158L12.668 2.95993Z" fill="url(#1752671328339-4791743_star-sparkle_existing_2_0fn5co6r9)"></path>
                <path d="M6.07314 2.88824L5.44324 1.30101C5.28398 0.8997 4.71603 0.89965 4.55669 1.30094L3.92644 2.88824C3.91951 2.90563 3.90577 2.91938 3.88838 2.9263L2.30091 3.55671C1.89969 3.71604 1.8997 4.28396 2.30092 4.44329L3.88838 5.0737C3.90577 5.08063 3.91951 5.09437 3.92644 5.11176L4.55668 6.69906C4.71602 7.10035 5.28397 7.1003 5.44324 6.69899L6.07314 5.11176C6.08008 5.0943 6.094 5.08061 6.11147 5.0737L7.69907 4.4433C8.10031 4.28398 8.10031 3.71602 7.69907 3.5567L6.11147 2.9263C6.094 2.91939 6.08008 2.9057 6.07314 2.88824Z" fill="url(#1752671328339-4791743_star-sparkle_existing_3_yonjglwpi)"></path>
                <path d="M23.5 16.25C23.5 16.9404 22.9404 17.5 22.25 17.5C21.5596 17.5 21 16.9404 21 16.25C21 15.5596 21.5596 15 22.25 15C22.9404 15 23.5 15.5596 23.5 16.25Z" fill="url(#1752671328339-4791743_star-sparkle_existing_4_dakdv8hg8)"></path>
                <path d="M18.2307 4.89743C18.949 4.93039 19.7944 5.15041 20.2727 5.89353L20.3548 6.03415C20.7312 6.74332 20.584 7.53845 20.3333 8.16404C20.3289 8.17492 20.3231 8.18536 20.3186 8.19626L18.2327 7.90329C18.3369 7.72619 18.4187 7.56496 18.4768 7.4199C18.578 7.16745 18.5847 7.03502 18.5813 6.98142C18.5338 6.95616 18.4108 6.90798 18.1399 6.89548C17.6948 6.87509 17.0493 6.96563 16.2268 7.20212C14.5943 7.67159 12.482 8.65321 10.303 10.0556C8.12386 11.4582 6.3547 12.974 5.25125 14.2656C4.69526 14.9164 4.34675 15.4662 4.18093 15.8799C4.07977 16.1323 4.07313 16.2646 4.07644 16.3183C4.12416 16.3436 4.24758 16.3918 4.51882 16.4043C4.76947 16.4157 5.08313 16.3909 5.45632 16.3261L5.09011 18.3867C4.86005 18.4047 4.63829 18.412 4.42703 18.4023C3.70884 18.3694 2.86334 18.1492 2.38504 17.4062C1.90698 16.6632 2.05709 15.803 2.32449 15.1357C2.60377 14.4391 3.10509 13.7002 3.73172 12.9668C4.99373 11.4897 6.92524 9.85163 9.22097 8.374C11.5166 6.89649 13.8071 5.81617 15.6741 5.27927C16.6011 5.01275 17.4811 4.86312 18.2307 4.89743Z" fill="url(#1752671328339-4791743_star-sparkle_existing_5_k25vmx6lf)"></path>
                <defs>
                  <linearGradient id="1752671328339-4791743_star-sparkle_existing_0_t5afbmhh9" x1="12.706" y1="7.902" x2="12.706" y2="18.386" gradientUnits="userSpaceOnUse">
                    <stop offset="2.78%" stopColor="#3B42FF"> </stop>
                    <stop offset="38.88%" stopColor="#BFECDC"> </stop>
                    <stop offset="74.77%" stopColor="#24AFFF"> </stop>
                    <stop offset="97.72%" stopColor="#4FD9C7"> </stop>
                  </linearGradient>
                  <linearGradient id="1752671328339-4791743_star-sparkle_existing_1_msjgnlsec" x1="12" y1="1.801" x2="12" y2="21.802" gradientUnits="userSpaceOnUse">
                    <stop offset="2.78%" stopColor="#3B42FF"> </stop>
                    <stop offset="38.88%" stopColor="#BFECDC"> </stop>
                    <stop offset="74.77%" stopColor="#24AFFF"> </stop>
                    <stop offset="97.72%" stopColor="#4FD9C7"> </stop>
                  </linearGradient>
                  <linearGradient id="1752671328339-4791743_star-sparkle_existing_2_0fn5co6r9" x1="12" y1="1.801" x2="12" y2="13.384" gradientUnits="userSpaceOnUse">
                    <stop offset="2.78%" stopColor="#3B42FF"> </stop>
                    <stop offset="38.88%" stopColor="#BFECDC"> </stop>
                    <stop offset="74.77%" stopColor="#24AFFF"> </stop>
                    <stop offset="97.72%" stopColor="#4FD9C7"> </stop>
                  </linearGradient>
                  <linearGradient id="1752671328339-4791743_star-sparkle_existing_3_yonjglwpi" x1="5" y1="1" x2="5" y2="7" gradientUnits="userSpaceOnUse">
                    <stop offset="2.78%" stopColor="#3B42FF"> </stop>
                    <stop offset="38.88%" stopColor="#BFECDC"> </stop>
                    <stop offset="74.77%" stopColor="#24AFFF"> </stop>
                    <stop offset="97.72%" stopColor="#4FD9C7"> </stop>
                  </linearGradient>
                  <linearGradient id="1752671328339-4791743_star-sparkle_existing_4_dakdv8hg8" x1="22.25" y1="15" x2="22.25" y2="17.5" gradientUnits="userSpaceOnUse">
                    <stop offset="2.78%" stopColor="#3B42FF"> </stop>
                    <stop offset="38.88%" stopColor="#BFECDC"> </stop>
                    <stop offset="74.77%" stopColor="#24AFFF"> </stop>
                    <stop offset="97.72%" stopColor="#4FD9C7"> </stop>
                  </linearGradient>
                  <filter id="1752671328339-4791743_star-sparkle_filter_0fcp7fp8u" x="-100%" y="-100%" width="400%" height="400%" filterUnits="objectBoundingBox" primitiveUnits="userSpaceOnUse">
                    <feGaussianBlur stdDeviation="2" x="0%" y="0%" width="100%" height="100%" in="SourceGraphic" edgeMode="none" result="blur"></feGaussianBlur>
                  </filter>
                  <clipPath id="1752671328339-4791743_star-sparkle_clipPath_mpnb7o899">
                    <path d="M10.6642 2.61922C11.2208 1.52816 12.7794 1.52811 13.336 2.61922L15.5069 6.87508C15.7292 7.31078 16.1505 7.6107 16.6349 7.67879L21.4083 8.34969C22.654 8.52479 23.1438 10.0637 22.2286 10.9268L18.839 14.1231C18.4713 14.4699 18.3034 14.979 18.3917 15.4766L19.2003 20.0372C19.4164 21.2558 18.1473 22.1966 17.0441 21.6358L12.6798 19.418C12.2526 19.2009 11.7476 19.2009 11.3204 19.418L6.95616 21.6358C5.85285 22.1967 4.58376 21.2559 4.79991 20.0372L5.60851 15.4766C5.69678 14.9789 5.52801 14.4699 5.16027 14.1231L1.77159 10.9268C0.856418 10.0637 1.34619 8.52479 2.59191 8.34969L7.36534 7.67879C7.84961 7.61064 8.27006 7.31071 8.4923 6.87508L10.6642 2.61922Z" fill="url(#1752671328339-4791743_star-sparkle_existing_1_msjgnlsec)"></path>
                  </clipPath>
                  <mask id="1752671328339-4791743_star-sparkle_mask_zol1sdobn">
                    <rect width="100%" height="100%" fill="#FFF"></rect>
                    <path d="M10.6642 2.61922C11.2208 1.52816 12.7794 1.52811 13.336 2.61922L15.5069 6.87508C15.7292 7.31078 16.1505 7.6107 16.6349 7.67879L21.4083 8.34969C22.654 8.52479 23.1438 10.0637 22.2286 10.9268L18.839 14.1231C18.4713 14.4699 18.3034 14.979 18.3917 15.4766L19.2003 20.0372C19.4164 21.2558 18.1473 22.1966 17.0441 21.6358L12.6798 19.418C12.2526 19.2009 11.7476 19.2009 11.3204 19.418L6.95616 21.6358C5.85285 22.1967 4.58376 21.2559 4.79991 20.0372L5.60851 15.4766C5.69678 14.9789 5.52801 14.4699 5.16027 14.1231L1.77159 10.9268C0.856418 10.0637 1.34619 8.52479 2.59191 8.34969L7.36534 7.67879C7.84961 7.61064 8.27006 7.31071 8.4923 6.87508L10.6642 2.61922Z" fill="#000"></path>
                  </mask>
                </defs>
              </g>
            </svg>
            </div>
            <CardHeader className="pb-4 relative z-10">
              <CardTitle className="text-xl font-bold text-card-foreground">
                AI Performance Evaluation
              </CardTitle>
              <CardTitle className="text-sm text-muted-foreground">
                Extracts notes, rhythm, and tempo from audio file, measures accuracy across
                multiple criteria, and generates accurate feedback
              </CardTitle>
            </CardHeader>
          </Card>

          {/* Audio-to-Sheet Conversion Card */}
          <Card className="bg-card border-border">
            <CardHeader className="pb-4">
              <CardTitle className="text-xl font-bold text-card-foreground">Audio-to-Sheet Conversion</CardTitle>
              <CardTitle className="text-sm text-muted-foreground"> Transform any trending song into playable sheet music. </CardTitle>
            </CardHeader>
          </Card>

          {/* Beginner Learning System Card - Full Width */}
          <Card className="bg-card border-border md:col-span-2">
            <CardHeader className="pb-4">
              <CardTitle className="text-xl font-bold text-card-foreground">Beginner Learning System</CardTitle>
              <CardTitle className="text-sm text-muted-foreground"> Structured lessons designed for first-time learners.</CardTitle>
            </CardHeader>
            <CardContent className="flex items-center justify-between">
              <div className="flex-1">
                <p className="text-muted-foreground text-sm leading-relaxed">
                  Structured lessons designed for first-time learners.
                </p>
              </div>
              {/* Node Diagram */}
              <div className="flex items-center space-x-4 ml-8">
                <div className="w-12 h-12 bg-chart-2 rounded-full flex items-center justify-center">
                  <div className="w-6 h-6 bg-background rounded-full"></div>
                </div>
                <div className="w-8 h-0.5 bg-muted"></div>
                <div className="w-12 h-12 bg-primary rounded-full flex items-center justify-center">
                  <div className="w-6 h-6 bg-background rounded-full"></div>
                </div>
                <div className="w-8 h-0.5 bg-muted"></div>
                <div className="w-12 h-12 bg-chart-3 rounded-full flex items-center justify-center">
                  <div className="w-6 h-6 bg-background rounded-full"></div>
                </div>
              </div>
            </CardContent>
          </Card>
        </div>
      </div>
    </div>
  )
}
